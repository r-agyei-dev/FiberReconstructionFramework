...
% Check to see what aspect of the code needs some redress and fix that                                                                             
...
for pp = 1
    ...
    initial_directory_name    =     '/scratch/brown/ragyei/CRITICAL_REGIONS_CELL/';
    fileFolder                =     sprintf('%s%d%s%d',initial_directory_name ,load_steps_idx(pp),'_files/XYZ_Recon_files_CONDENSED_N_',load_steps_idx(pp));
    cd(fileFolder)
    REFINED_XYZ               =     h5read(sprintf('%s%s%d%s',REFINED_nom_cell{sieve_indicator},'_xyz_',load_steps_idx(pp),'_.h5'),'/CELL_DATA/FIBER_RECON');

    cd('/scratch/brown/ragyei/Mutual_Comparison_GITHUB')
    [cell_var]                =     Extract_LIDX_fun(REFINED_XYZ);
 
    
% fb_labels_xyz = cell(1,numel(cell_var));    
    for hh = 1:numel(cell_var)
        fb_labels_xyz{ii}     =     REFINED_XYZ(cell_var{hh});
    end
