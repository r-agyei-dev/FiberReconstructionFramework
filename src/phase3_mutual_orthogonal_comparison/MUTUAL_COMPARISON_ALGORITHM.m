...
% REFINED_nom_cell      =     This is a prefix for the final saved .mat variable;
% save_path             =     Directory for the saved variables;
% mfile_direc           =     Directory for the current mfile for the mutual comparison cod;
% tomo_dataset_idx      =     This is the index (in a string)for the tomoset data taken for specimen;
% specimen_name         =     prefix for the specimen name;
% mat_file_directory    =     directory that contains folders of the reconstructions in the x,y,z directions respectively 

REFINED_nom_cell   =   {'REFINED_FIBERS_SIEVED'};
loop_vector_size_height  =     [1]';
tomo_dataset_idx         =     '0002';
% save_path                =     sprintf('%s%s%s%s',initial_directory_name,'\RECON_FILES\',specimen_name,tomo_dataset_idx);
results_path                =     '/Users/ronaldagyei/Desktop/PhD_Codes/results/SPLIT_FILES';
mfile_direc              =     pwd;
specimen_name            =     'specimen3_gfrp_';
% mat_file_directory       =     sprintf('%s%s%s','E:\BACKUP_3_24_2021\',specimen_name,tomo_dataset_idx);



for pp = 1:size(loop_vector_size_height,1)
    Center_Idx_prefix         =    {'Center_Idx_SIEVED','Center_Idx_UNSIEVED'};
    initial_directory_name    =    results_path;  % Only make changes to this path 
...
%         [REFINED_XYZ,REFINED_ZXY] = H5_Loader_Function(initial_directory_name,loop_vector_size_height(pp,1),sieve_indicator,REFINED_nom_cell,track_seq);
        [REFINED_XYZ,REFINED_ZXY] = H5_Loader_FUNC(initial_directory_name,tomo_dataset_idx,specimen_name,REFINED_nom_cell,track_seq);
...
%         [REFINED_XYZ,REFINED_YZX] = H5_Loader_Function(initial_directory_name,loop_vector_size_height(pp,1),sieve_indicator,REFINED_nom_cell,track_seq);
        [REFINED_XYZ,REFINED_YZX] = H5_Loader_FUNC(initial_directory_name,tomo_dataset_idx,specimen_name,REFINED_nom_cell,track_seq);
        cd(mfile_direc)
        disp('begin EPAD_FUNCTION_CODE_MASTER_VERSION_MUT')
...
%         [REFINED_ZXY,REFINED_YZX] = H5_Loader_Function(initial_directory_name,loop_vector_size_height(pp,1),sieve_indicator,REFINED_nom_cell,track_seq);
        [REFINED_ZXY,REFINED_YZX] = H5_Loader_FUNC(initial_directory_name,tomo_dataset_idx,specimen_name,REFINED_nom_cell,track_seq);
        cd(mfile_direc)
        disp('begin EPAD_FUNCTION_CODE_MASTER_VERSION_MUT')        
...
% LIDX       =   load(sprintf('%s%s%s%s%s',initial_directory_name,'\RECON_FILES\',specimen_name,tomo_dataset_idx,'\XYZ_REC_FILES\Center_Idx_UNSIEVED_xyz.mat'));
LIDX       =   load(sprintf('%s%s',initial_directory_name,'/XYZ_files/Center_Idx_UNSIEVED_xyz.mat'));
...
% CIDX       =   load(sprintf('%s%s%s%s%s',initial_directory_name,'\RECON_FILES\',specimen_name,tomo_dataset_idx,'\XYZ_REC_FILES\Cent_Coord_UNSIEVED_xyz.mat'));
CIDX       =   load(sprintf('%s%s',initial_directory_name,'/XYZ_files/Cent_Coord_UNSIEVED_xyz.mat'));
...
% LIDX       =   load(sprintf('%s%s%s%s%s',initial_directory_name,'\RECON_FILES\',specimen_name,tomo_dataset_idx,'\ZXY_REC_FILES\Center_Idx_UNSIEVED_zxy.mat'));
LIDX       =   load(sprintf('%s%s',initial_directory_name,'/ZXY_files/Center_Idx_UNSIEVED_zxy.mat'));
names      =    fieldnames(LIDX);
LIDX       =    LIDX.(names{1});
...
% CIDX       =   load(sprintf('%s%s%s%s%s',initial_directory_name,'\RECON_FILES\',specimen_name,tomo_dataset_idx,'\ZXY_REC_FILES\Cent_Coord_UNSIEVED_zxy.mat'));
CIDX       =   load(sprintf('%s%s%s%s%s',initial_directory_name,'/ZXY_files/Cent_Coord_UNSIEVED_zxy.mat'));
names      =   fieldnames(CIDX);
% CIDX     =   CIDX.(names{1});
...
% LIDX       =   load(sprintf('%s%s%s%s%s',initial_directory_name,'\RECON_FILES\',specimen_name,tomo_dataset_idx,'\YZX_REC_FILES\Center_Idx_UNSIEVED_yzx.mat'));
LIDX       =   load(sprintf('%s%s%s%s%s',initial_directory_name,'/YZX_files/Center_Idx_UNSIEVED_yzx.mat'));
names      =   fieldnames(LIDX);
LIDX       =   LIDX.(names{1});
...
% CIDX       =   load(sprintf('%s%s%s%s%s',initial_directory_name,'\RECON_FILES\',specimen_name,tomo_dataset_idx,'\YZX_REC_FILES\Cent_Coord_UNSIEVED_yzx.mat'));
CIDX       =   load(sprintf('%s%s%s%s%s',initial_directory_name,'/YZX_files/Cent_Coord_UNSIEVED_yzx.mat'));
names      =   fieldnames(CIDX);
% CIDX     =   CIDX.(names{1});
...
nom_file_prefix = 'FULL_CRITICAL_VOL_UNLUMPED_';
[variable_out]  = MUTUAL_MAIN_HDF_CREATOR(TEMP_MAT,nom_file_prefix,tomo_dataset_idx,initial_directory_name);
...
nom_file_prefix = 'FINAL_OPTIMIZED_';
...
nom_file_prefix = 'FINAL_OPTIMIZED_CLEANED_';
% PROTRUDE_CORRECTOR = h5read(sprintf('%s%s%d%s%d%s%d%s',results_path,'CRITICAL_REGIONS_CELL/',loop_vector_size_height(pp,1),'_files/XYZ_Recon_files_CONDENSED_N_',loop_vector_size_height(pp,1),'/Actual_Binary_Image_vol_1',loop_vector_size_height(pp,1),'_.h5')...
%                             ,'/CELL_DATA/FIBER_RECON');
PROTRUDE_CORRECTOR = h5read(sprintf('%s%s%s%s%s',results_path,'/XYZ_files','/Actual_Binary_Image_vol_1',tomo_dataset_idx,'_.h5')...
                            ,'/CELL_DATA/FIBER_RECON');
