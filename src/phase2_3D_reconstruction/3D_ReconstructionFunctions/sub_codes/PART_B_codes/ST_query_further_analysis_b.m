% ST_QUERY_FURTHER_ANALYSIS_B
% This function performs a detailed analysis of a query fiber in a 3D fiber
% reconstruction context. It resolves ambiguities related to overlapping fibers,
% in-plane distances, angular differences, and slice projections. 
% The function refines the candidate query fibers and updates their associated
% main result matrices based on several checks: slice overlap, 3D orientation,
% in-plane distance, and fiber uniqueness.
%
% Inputs:
%   query_var                       - Index of candidate query fiber(s)
%   crude_bin_mat                    - Flag indicating if overlap analysis is used
...
%   radius                           - Radius for in-plane distance calculations
...
%   circlePixels_points_IDX           - Linear indices of circle pixels for overlap

function [varargout] = ST_query_further_analysis_b(varargin)
...
%% STEP 1: Update main_res and main_res_extract for the query fiber
if ~isempty(query_var)
    
    if crude_bin_mat == 1
        % If overlap analysis is used, determine which slice height to use
        ...
        % Narrow down indices corresponding to query fiber height
        query_height = cellfun(@(x)x(end), z_TMP(query_var), 'uni', 0);
        LIN_extract_lim = cellfun(@(x,y,z)y(ismember(x,z)), zt, LIN_extract_lim, query_height, 'uni',0);
        query_height = cell2mat(query_height);
        
        % Compute overlaps with projected main_res slices
        ...
        % Placeholder for non-overlap-based correction of rogue fibers
    end
end

%% STEP 2: Project to seed fiber and extract coordinates for in-plane distance checks
if ~isempty(query_var)
    ...
    for gg = 1:numel(query_var)
        ...
        if ~isempty(probe_varN) && crude_bin_mat == 1
            % Select points matching probe fiber z-position
            main_res_extractN = main_resN(ismember(main_resN(:,3), z_TMP{probe_varN}(1)),:);
        else
            main_res_extractN = [];
        end
    ...
    % Remove query fibers that failed projection
    ...
    %% STEP 2A: Angle check between seed and query fibers
    ...
    %% STEP 2B: In-plane centroid distance check
    cent_cell_temp = {Centroid_MID_cell_LUMP_TMP{query_var}};
    dist_vec = cell2mat(cellfun(@(x)norm(main_res_extract(1,1:2) - x(end,1:2)), cent_cell_temp,'uni',0));
    
    cent_cell_tempN = Centroid_MID_cell_LUMP_TMP{c_f};
    dist_vecN = cell2mat(cellfun(@(x)norm(x(1,1:2) - cent_cell_tempN(1,1:2)), main_res_extractV,'uni',0));
...
% If multiple candidates remain, pick based on angular size
...
% If still multiple, pick the tallest fiber
if numel(query_var) > 1
